...
function extractWindowExports(content: string): Array<{ name: string; type: string }> {
  ...
  while ((match = windowExportPattern.exec(content)) !== null) {
    ...
    const isFunction = functionPattern.test(content.slice(match.index, match.index + 100)) ||
...
function extractFunctionSignatures(content: string, exportName: string): string | undefined {
  ...
  const patterns = [
    ...
    new RegExp(`\\(window as any\\)\\.${exportName}\\s*=\\s*function\\s*([A-Za-z_][A-Za-z0-9_]*)\\s*\\(([^)]*)\\)`, 'i')
  ];
...
function main() {
  ...
  console.log(`✓ Generated datastar-manifest.json (${Object.keys(manifest.exports).length} categories, ${Object.values(manifest.exports).reduce((sum, cat) => sum + Object.keys(cat).length, 0)} exports)`);
