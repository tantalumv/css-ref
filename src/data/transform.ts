...
export const transform: CSSProperty[] = [
  {n:'transform',c:'Transform',d:'Applies 2D or 3D transformations — rotate, scale, translate, skew, and matrix.',s:{ch:1,ff:1,sf:1,ed:1},i:'wide',x:'transform: rotate(45deg) scale(1.2) translateX(20px)',m:'transform',
  ...
  {n:'translate',c:'Transform',d:'Individual transform property for moving an element, composable with rotate and scale.',s:{ch:1,ff:1,sf:1,ed:1},i:'b2022',x:'translate: 50px 100px | 50% | none',m:'translate',
  ...
  {n:'rotate',c:'Transform',d:'Individual transform property for rotating an element without affecting other transforms.',s:{ch:1,ff:1,sf:1,ed:1},i:'b2022',x:'rotate: 45deg | 1 0 0 45deg | none',m:'rotate',
   demo:`<div style="display:flex;gap:10px;align-items:center;padding:10px">${[0,30,60,90,135].map(d=>`<div style="width:28px;height:28px;background:#14b8a6;border-radius:3px;rotate:${d}deg;opacity:${0.4+d/180}"></div>`).join('')}</div>`},

  {n:'scale',c:'Transform',d:'Individual transform property for scaling an element along X, Y, and Z axes.',s:{ch:1,ff:1,sf:1,ed:1},i:'b2022',x:'scale: 1.5 | 0.5 2 | none',m:'scale',
   demo:`<div style="display:flex;gap:12px;align-items:center;padding:14px">${[['.5','.5x'],['1','1x'],['1.3','1.3x'],['1.6','1.6x']].map(([s,l])=>`<div style="width:28px;height:28px;background:#14b8a6;border-radius:3px;scale:${s};display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:700;color:#fff">${l}</div>`).join('')}</div>`},

  {n:'transform-style',c:'Transform',d:'Sets whether children of a transformed element are positioned in 3D or flattened 2D space.',s:{ch:1,ff:1,sf:1,ed:1},i:'wide',x:'transform-style: flat | preserve-3d',m:'transform-style',
