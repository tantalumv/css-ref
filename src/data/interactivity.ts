...
export const interactivity: CSSProperty[] = [
  {
    ...
    m: "cursor",
    demo: `<div style="display:flex;flex-wrap:wrap;gap:5px;padding:8px">${[
      ["pointer", "🖱️"],
      ["text", "I"],
      ["grab", "✋"],
      ["not-allowed", "🚫"],
      ...
      .map(
        ([c, i]) =>
          `<div style="cursor:${c};background:#fee2e2;border:2px solid #ef4444;border-radius:4px;padding:4px 8px;font-size:10px;font-weight:700;color:#b91c1c">${i} ${c}</div>`,
      )
      .join("")}</div>`,
  },
