...
export function borderDemo(
  ...
  return styledBox(label, {
    border,
    background: "#f0f0f0",
    width: size.width || "80px",
    height: size.height || "60px",
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    color: "#333",
...
/**
 * Create a multi-box demo for showing multiple states
 */
export function multiBoxDemo(
  items: Array<{ content: string; styles: BoxOptions }>,
  containerOptions?: FlexContainerOptions,
): string {
  const boxes = items.map((item) => styledBox(item.content, item.styles));
  return flexContainer(boxes, containerOptions);
}
